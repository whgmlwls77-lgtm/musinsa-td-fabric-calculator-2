...
### 8.1. 답변 전 자가 검증 체크리스트 (사용자 피드백 #38~#39 — 영구 박제)
...
- [ ] **[신규 #39] 모호 표현 0건?**
      - 금지 정도 부사: "살짝", "약간", "소폭", "다소", "꽤", "매우", "많이", "적게"
      - 금지 근사 표현: "대략", "약", "정도", "쯤", "가량"
      - 금지 이중 표현: "~할 수도 있고 ~할 수도 있다"
      - 허용: 정확한 수치 (%, mm, cm) or "실측 X.XX%" or "시제품 검증 후 확정"
- [ ] **라벨** (✅/❌/⚠️/❓) 박혀있나?
...
| 2026-07-24 | §8.1 모호 표현 금지 항목 추가 (피드백 #39) | (다음 commit) |
| 2026-05-10 | 식서 가로 통일 (X 그대로/Y 회전) + 마카 갯수 placements 라벨 정정 — 사장님 시각 검증 ✅ | (다음 commit) |
| 2026-07-23 | Task #38 (b~e) 요척 PDF/Excel 재구성 (sparrow SVG 임베드·한글 폰트 주입·원단별 시트·레이아웃 통일) | `9ded99a` |
| 2026-07-23 | Task #38-f [A] 축율 판정 표시/판정 정밀도 불일치 fix — `round(actual,1) > round(declared,1)` (실측 <0.05% = 화면 "+0.0%" → 정상) | `bb666d5` |
| 2026-07-24 | Task #38-f [B] **BLK_5_1 +0.2% 가로 = raw bbox 유지 (수정 X)** — 곡선 조각 극점 0.38mm(정점 31→36 샘플링 차), 조각은 실질 축소(면적 -0.3%/세로 -3.3%). 회전 아님(x_min 동일). 오매칭 아님(block_name 매칭). 원칙 #1 raw 우선 | (문서 커밋) |
| 2026-07-24 | Task #38-g **발제** — 식서(grain) 축 기준 치수 측정 도입 (raw bbox 극점 민감성 근본 해결 · 본질 #1 "식서=요척 핵심" 정렬) · 별도 설계/승인 대기 | (예정) |
| 2026-07-27 | Task #38-g + #38-h Step 1 **완성** — 축율 측정 근본 재설계: **면적=판정 · 4변+정중앙 세로/가로=근거 · 정합성=이상 감지 · 매칭 shape 게이트**. 각도 4코너(재샘플링 무관 · 28_Small -7.27%→-0.00%) · G-F 폐기(좌우평균 정확). UI 카드+색막대·총계 5카드·좌우 6:4·이미지 400px 스케일·A3 그리드. 회귀 393 passed. 사장님 시각 검증 ✅ | (통합 커밋) |

---

*마지막 갱신: 2026-07-27 (Task #38-g + #38-h Step 1 완성 — 면적 우선 판정 + 각도 4코너 + 정중앙 세로/가로 · 사장님 시각 검증 통과 · PATTERN_PREP_GUIDE v2.1 §12~§14 박제)*
