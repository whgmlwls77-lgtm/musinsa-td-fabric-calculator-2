# 원단 요척 산출 시스템 — DXF 표기 규칙 사양서 (v2.0 정식)

> **대상**: 협력사 패턴사 / 사내 소싱팀
> **목적**: 본 시스템이 DXF 를 정확히 인식하도록 **표기 형식**을 사양 수준으로 명시
> **상태**: **v2.0 정식 (2026-05-07 승격)** — 이전 v1.5 폐기
> **작성일**: 2026-05-04 / 정식 승격: 2026-05-07
...
## §12. 축율 검증 시스템 (v2.1 신설 — Task #38-g/h · 2026-07-27)

> PP(축율 미반영) vs 메인(축율 반영) 패턴 조각별 축율 검증. 사장님 원 개념 완전 구현:
> **면적 = 판정 · 4변+정중앙 = 근거 · 정합성 = 이상 감지 · 매칭 = 신뢰도**.

### §12.1 판정 로직 (면적 우선)
- **판정 기준 = 면적 확대율만** (조각 총 크기 변화).
  - 🚨 상한초과: `|면적 확대율| > 5%` (INDUSTRY_MAX · 양방향)
  - ⚠️ 신고초과: `면적 확대율 > 신고` (declared · 확대 방향)
  - ✅ 정상: 그 외
- **round tolerance** (Task #38-f): `round(actual,1) > round(declared,1)` — 화면 표시(+0.0%)와 판정 일치.
- 세로/가로/4변 = **판정 안 함 · 근거 정보만**.

### §12.2 근거 정보 (측정 7종)
식서(grain) 축 정렬 후 **각도 기반 4코너**(정점 재샘플링 무관) 측정:
- 4변: 좌변·우변(세로 근거) · 상변·하변(가로 근거)
- 세로 축율 = (좌변+우변)/2 · 가로 축율 = (상변+하변)/2
- **정중앙 세로/가로** = centroid 통과 세로/가로선 ∩ 조각 (shapely) — 중심 실제 span
- 원형/타원 = 세로/가로 지름
- 대칭성 진단: 좌우·상하 편차 ≥ 0.5%p 알림

### §12.3 정합성 검증 (이상 감지)
- 이론 면적 = `(1+세로/100)(1+가로/100)-1` vs 실제 면적, 차 > 2%p → **⚠️ 이상 신호**.
- 원인 힌트: 정점 재샘플링 차 N점 · 4코너 정확도 낮음 · 육안 확인 권장.

### §12.4 매칭 로직 (block_name + shape similarity 게이트)
- 1차 block_name 매칭 → 도형 유사도 게이트: `< 0.5` 무효(재매칭) · `≥ 0.85` 확정 · `0.5~0.85` 의심.
- 신뢰도 배지: ✅ 이름+도형 / 🔷 도형 / ⚠️ 의심. (좌표 부재 시 이름 신뢰)
- 크기 순 fallback 폐기 (Task #35).

### §12.5 실행 흐름
파일 업로드(PP슬롯·메인슬롯) → 기준 사이즈 → 검증 대상 원단 → 신고 축율 입력 → **▶️ 축율 검증 실행** (버튼 클릭 시에만 계산).

### §12.6 UI
- 조각별 **경계선 카드** + 판정별 좌측 색막대(🚨빨강/⚠️노랑/✅초록) · 판정 아이콘 확대.
- 총계 **5카드**: PP 총면적 · 메인 총면적 · 면적 확대율 · **평균 세로 · 평균 가로**.
- 상세 열림 = `st.columns([6,4])`: 좌 데이터(12px 컴팩트) / 우 오버랩 이미지.
- 오버랩: 측정 4변(파랑=세로/초록=가로) + 정중앙 십자선(주황) · **조각 실제 크기 비율 스케일(최대 400px)**.
- 조각 정렬 = 면적 큰 순.

### §12.7 다운로드
- PDF: **A3 가로 한 페이지 그리드** (2×4 · 8조각/페이지 · 오버랩 이미지 + 축율 요약).
- Excel: 요약 / 원단별요약 / 조각별상세(모든 조각·14컬럼) / 오버랩이미지.

### §13. 로드맵 (Task #38-h Step 2/3 이후)
...
| 시점 | 결정 |
|------|------|
| 2026-07-26 | 판정 = 면적 우선 · 세로/가로 = 근거 · 정합성 검증 신설 |
| 2026-07-26 | 매칭 shape similarity 게이트 (신뢰도 배지) · 실행 버튼 |
| 2026-07-27 | 각도 기반 4코너 (재샘플링 무관 · 28_Small -7.27%→-0.00%) |
| 2026-07-27 | 정중앙 세로/가로 도입 · G-F 폐기 (좌우평균이 정확) |
| 2026-07-27 | UI 카드+색막대 · 총계 5카드 · 좌우 6:4 · 이미지 400px 스케일 · A3 그리드 |

---

**버전**: 2.1 · **작성일**: 2026-05-04 (v2.0) → 2026-07-27 (v2.1 축율 검증 §12~§14 신설)
**근거 데이터**: `output/mmaps003_raw_audit_2026-05-04.md` · MWEKS9D02 / MWFTL8A05 검증
